...
class TestFunction(private val name: String, val z: (x: Double, y: Double) -> Double) {
    ...
    fun matrix(resolution: Int): List<List<Double>> {
    ...
    fun image(resolution: Int): Image {
        val img = BufferedImage(resolution, resolution, 1)
        val matrix = this.matrix(resolution = resolution)

        for (y in 0 until resolution) {
            for (x in 0 until resolution) {
                img.setRGB(x, y, Color.HSBtoRGB(0f, 0f, 1 - matrix[y][x].toFloat()))
            }
        }
